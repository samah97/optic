...
class GUMP
{
    ...
    protected function validate_alpha_dash($field, $input, $param = null)
    {
        ...
        if (!preg_match('/^([a-zÃ€Ã�Ã‚ÃƒÃ„Ã…Ã‡ÃˆÃ‰ÃŠÃ‹ÃŒÃ�ÃŽÃ�Ã’Ã“Ã”Ã•Ã–ÃŸÃ™ÃšÃ›ÃœÃ�Ã Ã¡Ã¢Ã£Ã¤Ã¥Ã§Ã¨Ã©ÃªÃ«Ã¬Ã­Ã®Ã¯Ã°Ã²Ã³Ã´ÃµÃ¶Ã¹ÃºÃ»Ã¼Ã½Ã¿_-])+$/i', $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    protected function validate_alpha_space($field, $input, $param = null)
    {
        ...
        if (!preg_match("/^([0-9a-zÃ€Ã�Ã‚ÃƒÃ„Ã…Ã‡ÃˆÃ‰ÃŠÃ‹ÃŒÃ�ÃŽÃ�Ã’Ã“Ã”Ã•Ã–ÃŸÃ™ÃšÃ›ÃœÃ�Ã Ã¡Ã¢Ã£Ã¤Ã¥Ã§Ã¨Ã©ÃªÃ«Ã¬Ã­Ã®Ã¯Ã°Ã²Ã³Ã´ÃµÃ¶Ã¹ÃºÃ»Ã¼Ã½Ã¿\s])+$/i", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    protected function validate_valid_persian_name($field, $input, $param = null)
    {
        ...
        if (!preg_match("/^([Ø§ Ø¢ Ø£ Ø¥ Ø¨ Ù¾ Øª Ø« Ø¬ Ú† Ø­ Ø® Ø¯ Ø° Ø± Ø² Ú˜ Ø³ Ø´ Øµ Ø¶ Ø· Ø¸ Ø¹ Øº Ù� Ù‚ Ú© Ùƒ Ú¯ Ù„ Ù… Ù† Ùˆ Ø¤ Ù‡ Ø© ÛŒ ÙŠ Ø¦ Ø¡ Ù‘ ÙŽ Ù� Ù� Ù‹ Ù� ÙŒ Ù’\x{200B}-\x{200D}])+$/u", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    protected function validate_valid_eng_per_pas_name($field, $input, $param = null)
    {
        ...
        if (!preg_match("/^([A-Za-zÃ€Ã�Ã‚ÃƒÃ„Ã…Ã‡ÃˆÃ‰ÃŠÃ‹ÃŒÃ�ÃŽÃ�Ã’Ã“Ã”Ã•Ã–ÃŸÃ™ÃšÃ›ÃœÃ�Ã Ã¡Ã¢Ã£Ã¤Ã¥Ã§Ã¨Ã©ÃªÃ«Ã¬Ã­Ã®Ã¯Ã±Ã°Ã²Ã³Ã´ÃµÃ¶Ã¹ÃºÃ»Ã¼Ã½Ã¿'\- Ø§ Ø¢ Ø£ Ø¥ Ø¨ Ù¾ Øª Ù¼ Ø« Ú… Ø¬ Ú† Ø­ Ú� Ø® Ø¯ Ú‰ Ø° Ø± Ú“ Ø² Ú– Ú˜ Ø³ Ø´ Úš Øµ Ø¶ Ø· Ø¸ Ø¹ Øº Ù� Ù‚ Ú© Ú« Ùƒ Ú¯ Ù„ Ù… Ù† Ú¼ Ùˆ Ø¤ Ù‡ Ø© ÛŒ ÙŠ Û� Û� Ø¦ Ø‹ Ø¡ Ù‘ ÙŽ Ù� Ù� Ù‹ Ù� ÙŒ Ù’ \x{200B}-\x{200D} \s])+$/u", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    protected function validate_valid_persian_digit($field, $input, $param = null)
    {
        ...
        if (!preg_match("/^([Û°Û±Û²Û³Û´ÛµÛ¶Û·Û¸Û¹Ù Ù¡Ù¢Ù£Ù¤Ù¥Ù¦Ù§Ù¨Ù©])+$/u", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    protected function validate_valid_persian_text($field, $input, $param = null)
    {
        ...
        if (!preg_match("/^([Ø§ Ø¢ Ø£ Ø¥ Ø¨ Ù¾ Øª Ø« Ø¬ Ú† Ø­ Ø® Ø¯ Ø° Ø± Ø² Ú˜ Ø³ Ø´ Øµ Ø¶ Ø· Ø¸ Ø¹ Øº Ù� Ù‚ Ú© Ùƒ Ú¯ Ù„ Ù… Ù† Ùˆ Ø¤ Ù‡ Ø© ÛŒ ÙŠ Ø¦ Ø¡ Ù‘ ÙŽ Ù� Ù� Ù‹ Ù� ÙŒ \. \/ \\ = \- \| \{ \} \[ \] Ø› : Â« Â» ØŸ > < \+ \( \) \* ØŒ Ã— Ùª Ù« Ù¬ ! Û°Û±Û²Û³Û´ÛµÛ¶Û·Û¸Û¹Ù Ù¡Ù¢Ù£Ù¤Ù¥Ù¦Ù§Ù¨Ù©\x{200B}-\x{200D} \x{FEFF} \x{22} \x{27} \x{60} \x{B4} \x{2018} \x{2019} \x{201C} \x{201D} \s])+$/u", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
    ...
    protected function validate_valid_pashtu_text($field, $input, $param = null)
    {
        ...
        if (!preg_match("/^([Ø§ Ø¢ Ø£ Ø¨ Ù¾ Øª Ù¼ Ø« Ú… Ø¬ Ú† Ø­ Ú� Ø® Ø¯ Ú‰ Ø° Ø± Ú“ Ø² Ú– Ú˜ Ø³ Ø´ Úš Øµ Ø¶ Ø· Ø¸ Ø¹ Øº Ù� Ù‚ Ú© Ú« Ù„ Ù… Ù† Ú¼ Ùˆ Ø¤ Ù‡ Ø© ÛŒ Û� Û� ÙŠ Ø¦ Ø¡ Ù’ ÙŒ Ù� Ù‹ Ù� Ù� ÙŽ Ù‘ Ø‹ \. \/ \\ = \- \| \{ \} \[ \] Ø› : Â« Â» ØŸ > < \+ \( \) \* ØŒ Ã— Ùª Ù« Ù¬ ! Û°Û±Û²Û³Û´ÛµÛ¶Û·Û¸Û¹Ù Ù¡Ù¢Ù£Ù¤Ù¥Ù¦Ù§Ù¨Ù© \x{200B}-\x{200D} \x{FEFF} \x{22} \x{27} \x{60} \x{B4} \x{2018} \x{2019} \x{201C} \x{201D} \s])+$/u", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
